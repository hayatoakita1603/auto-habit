...
jest.mock("../../src/services/achievementService", () => ({
	getAchievementsByMonth: (...args: unknown[]) =>
		mockGetAchievementsByMonth(...args),
}));
...
describe("useCalendar", () => {
	...
	it("達成日セットを取得してloading: falseになる", async () => {
		mockGetAchievementsByMonth.mockResolvedValue(
			new Set(["2026-02-10", "2026-02-15"]),
		);

		const { result } = renderHook(() => useCalendar(mockUser));
		await act(async () => {});

		expect(result.current.achievedDates).toEqual(
			new Set(["2026-02-10", "2026-02-15"]),
		);
	...
	it("月が変わると再フェッチする", async () => {
		...
		expect(mockGetAchievementsByMonth).toHaveBeenLastCalledWith(
			"user-1",
			2026,
			1,
		);
